...
import numpy as np
import math
from Summary import stat_correct


# 输入历史的炉子信息，为过去的每个炉子赋予加权指数偏差，同时返回下一天炉子的估计偏差
def correct_predict(ST, name, gate, theta):
    # 按日期分组
...
def correct_opti_one(ST, stand, name):
    ...
    # 寻找最优gate
        ...
        for gate in gate_arange:
            next_pre_error = correct_predict(ST, name, gate, 0.0707)
            # 进行修正的估计 后的数据统计
            d_error_ave, d_max, acc, acc_p, st_per, gr_per = stat_correct(ST, stand)
            v.append(acc_p[name])
    else:
...
def correct(ST, stand, name, gate, theta):
    next_pre_error = correct_predict(ST, name, gate, theta)
    # 进行修正的估计 后的数据统计
    d_error_ave, d_max, acc, acc_p, st_per, gr_per = stat_correct(ST, stand)
