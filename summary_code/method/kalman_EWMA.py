...
def kalman_sum_EWMA_by_date(ST, name, gate, theta):
    # 按日期分组
    ...
    for d in sorted_dates:
        ...
        for st in value_dict[d]:
            ...
            if (correct_error is not None) and (pre_error is not None):
                getattr(st, name)[5] = (correct_error + pre_error) / 2.0

        if (correct_error is not None) and (pre_error is not None):
            pre_error_list.append((correct_error + pre_error) / 2.0)
        ...
        if (correct_error is None) and (pre_error is None):
            if e_num != 0:
                ...
                pre_error_list.append((correct_error + ave_error[0]) / 2.0)
        else:
            if e_num != 0:
                ave_error.append(e_sum / e_num)  # 计算一天的平均偏差
    ...
    if (correct_error is None) or (pre_error is None):
        ...
        final_error = (correct_error + pre_error) / 2.0
...
def kalman_sum_EWMA_by_num(ST, name, gate, theta):
    # 排序  按照日期从小到大  序号从小到大
    ...
    pre_error_list = []  # 保存所有的预测偏差(kalman + EWMA)/2
    kf = None  # 卡尔曼滤波器
...
def kalman_EWMA_correct(ST, stand, name, gate, theta):
    next_pre_error, pre_error_list = kalman_sum_EWMA_by_num(ST, name, gate, theta)
    # 进行修正的估计 后的数据统计
    d_error_ave, d_max, acc, acc_p, st_per, gr_per = stat_correct(ST, stand)
